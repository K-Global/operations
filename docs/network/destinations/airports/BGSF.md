...
BGSF sits at the head of **Søndre Strømfjord (Kangerlussuaq Fjord)**, roughly 190 km inland from the west coast — arrivals and departures track up/down a long fjord with rising terrain on both sides. This is the airport's defining hazard: there is **no easy circling option**, and a late runway change (e.g. wind swings to favour RWY 27, which has no published instrument approach) can force a visual/circling manoeuvre inside the fjord walls at night or in marginal weather. MSA/grid-MORA in this sector is driven by the Greenland icecap dome to the east (~10,500 ft, see  §7) — respect published MSA sectors and do not descend early on the assumption that "inland = flat." Missed-approach and driftdown planning must assume terrain on both sides of the inbound track, not just ahead.

### 3.2 Airborne conflict / traffic

Traffic density is low (regional Air Greenland service plus occasional widebody diversions/technical stops). Control is **procedural (non-radar)** since Sondrestrom's radar service ended ~Nov 2025 — the workload driver is compulsory position reporting, not conflict density. See  §3/§10 for the Nuuk FIR procedural-contingency framework.
...
|---|---|---|---|---|---|---|---|
| 09 | 2,810 x 60 m (9,219 x 197 ft) | Asphalt, hard-surface, lighted; PCN not published / verify | Not published / verify | Not published / verify | Not published / verify | Not published / verify | THR elevation ≈100 ft; instrument-approach runway (LOC/RNP/NDB). No displaced threshold reported. |
| 27 | 2,810 x 60 m (9,219 x 197 ft) | Asphalt, hard-surface, lighted; PCN not published / verify | Not published / verify | Not published / verify | Not published / verify | Not published / verify | THR elevation ≈165 ft; **no published instrument approach** — visual/circling only. RNP 1 SID published for this direction. |

<!-- Declared distances (TORA/TODA/ASDA/LDA) could not be corroborated from a reachable primary source at build time — see §18. Overall runway length is well-corroborated (OurAirports, SkyVector, AirportGuide, Wikipedia all concordant at 2,810 m / 9,219 ft). -->
...
> **Live weather — pull at planning (T-2h):** METAR, TAF, TREND, SIGMET/AIRMET, space-weather HF/GNSS advisories (see  §11). Not stored here.
...
## Sources & References
*Public URLs only. Cite origin + effective + retrieved dates. Subscription/in-sim material may inform content but is not listed here. See  for the tier hierarchy.*
...
- **OurAirports — BGSF** — elevation, coordinates, runway dimensions — https://ourairports.com/airports/BGSF/ and https://ourairports.com/airports/BGSF/runways.html (retrieved 2026-07-25).
- **SkyVector — BGSF/Kangerlussuaq Airport** — coordinates, runway/threshold data, navaids, tower/emergency frequencies — https://skyvector.com/airport/BGSF/Kangerlussuaq-Airport (retrieved 2026-07-25).
...
- Cross-reference:  §7 (icecap terrain/MORA), §8 (diversion table, fuel-freeze), §11 (space-weather).
