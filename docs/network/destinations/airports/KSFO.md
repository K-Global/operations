...
| RFF category | **Index E** (SF Fire Dept / cross-ref — confirm against AD-2) |
...
| Item | Value | Flag |
|---|---|---|
| ATS hours | H24 assumed for a major hub — not explicitly confirmed in reachable extract | |
...
*Source: FAA Chart Supplement (d-TPP), AIRAC cycle 2607 eff. 2026-07-09, as mirrored by SkyVector — treated as primary-equivalent (retrieved 2026-07-25). PCR is the US-domestic equivalent presentation of ICAO PCN; confirm ICAO-format PCN if required for a non-US operator manual. All distances in feet. **RWY 01R/19L reported closed for repaving as of the build date (approx. late Mar–early Oct 2026) — verify current status via NOTAM.***
...
| Approach / Departure | NORCAL Approach/Departure | 128.325 / 133.95 / 134.5 / 120.9 / 135.1 (sector/STAR/DP-specific — multiple discrete freqs published per procedure) | H24 (assumed) | Radar; freq assignment is procedure-specific (e.g. BDEGA STAR 133.95/317.6) — confirm current chart |
| Centre / FIR | Oakland Center (ZOA) | Not confirmed in reachable extract | H24 (assumed) | |

*Emergency 121.5. UNICOM 122.95. Source: FAA Chart Supplement via SkyVector mirror (cycle 2607 eff. 2026-07-09), retrieved 2026-07-25 — treated as primary-equivalent; centre/FIR frequency and exact ATIS arrival/departure split not visible in the retrieved extract, flagged for AIP/chart confirmation.*
...
| ILS/DME 28R | GWQ | 111.70 | H24 | **CAT II/III** — secondary, verify ident/freq |
| ILS/LOC 28L | Not confirmed (secondary sources conflict) | ≈109.55 | H24 | **SA CAT II** — verify ident/freq on current chart |
| ILS/LOC 19L | Not confirmed (secondary sources conflict) | ≈108.9 | H24 | CAT I — verify ident/freq on current chart |
...
*ILS idents/frequencies from tier-4 sources conflicted between providers during research — treat all ILS ident/freq entries above as pending current-AIRAC chart confirmation. VOR/DME SFO 115.80 and the nearby VORs are corroborated across SkyVector/FAA Chart Supplement mirror and OurAirports.*
...
- **Typical role for us:** Destination (OFP KAY09MA EDDF–KSFO).
- **Nearest suitable alternates:** **KOAK** Oakland Intl (~10 NM ) — same marine-layer weather system, limited independent value in fog; **KSJC** San Jose Intl (~26 NM ). Verify suitability, runway/RFF adequacy and current minima; cross-check against current dispatch data.
