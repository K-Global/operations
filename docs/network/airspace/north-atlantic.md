...
- **NAT HLA (High Level Airspace):** the volume **between FL285 and FL420 inclusive** (NAT Doc 007 Foreword & §1.1.1). Comprises the OCAs of **Bodø Oceanic, Gander Oceanic, New York Oceanic East (north of 27°N), Reykjavik, Santa Maria and Shanwick**, **excluding** the Shannon (SOTA) and Brest (BOTA) Ocean Transition Areas. Operation inside NAT HLA requires a formal **NAT HLA approval** (the term that replaced "MNPS") **and RVSM approval** (§1.1).
- **OCAs / FIRs & controlling ATSU:**
  - **Reykjavik (BIRD)** — Reykjavik ACC, Isavia. Also the Reykjavik CTA, which extends over the upper Greenland (Nuuk) airspace north of 63°30′N — see the Arctic briefing.
  - **Gander Oceanic (CZQX)** — Gander OAC ("Gander Centre"), NAV CANADA. **Produces the nighttime/eastbound OTS.**
  - **Shanwick Oceanic (EGGX)** — Prestwick (Shanwick OAC), with Shanwick Radio (Ireland) for voice. **Produces the daytime/westbound OTS.**
  - **Bodø Oceanic (ENOB)** — Norway. **Santa Maria Oceanic (LPPO)** — Portugal.
  - **New York Oceanic East** — NAT HLA only **north of 27°N**; **excluded from the datalink mandate** (§4).
...
- **Principal route structure — Organised Track System (OTS):** a grid of agreed tracks built **twice daily** for the two alternating unidirectional peaks (§2.1.1):
  - **Daytime / WESTbound OTS** — produced by **Shanwick (Prestwick)**; validity **1130–1900 UTC at 30°W**.
  - **Nighttime / EASTbound OTS** — produced by **Gander**; validity **0100–0800 UTC at 30°W**.
  - Typical OTS levels **FL340–FL400** (§2.1.2). The publishing OAC incorporates New York, Reykjavik, Bodø and Santa Maria tracks.
- **NAT Track Message / TMI:** tracks are promulgated with a **Track Message Identification (TMI)** number (= day-of-year, e.g. "TMI 326"); a revised message takes an **alpha suffix** when coordinates, waypoints or levels change (§4.1.7). PBCS/RNP4 tracks are spaced through **whole and half degrees of latitude** (§1.9.2).
- **Track use is NOT mandatory** — random routing is permitted (§2.1.4). The filed route is the SimBrief/current-AIRAC-validated string; the **oceanic clearance** assigns the actual track/random route, level and Mach.
...
- **Coast-in / coast-out:** driftdown or depressurisation escape near the landmass ends (Iceland, Greenland, Canadian Atlantic) reaches significant terrain — the binding constraint is the **Greenland icecap dome (~10,500 ft)** on the high-latitude side. Full grid-MORA and driftdown picture in the Arctic briefing §7.
...
- **Conflict / prohibited / risk zones:** none within the open-ocean NAT OCAs — benign for overflight-permit and conflict-zone purposes.
- **Danger / restricted / military areas:** occasional ALTRV / military activity and oceanic danger areas by NOTAM (range activity near the coasts) — pull live at §11.
...
Globally-standardised oceanic in-flight contingency (in force since 5 Nov 2020), per **NAT Doc 007 Ch.10** and reproduced verbatim in **FAA AIP ENR 7.3**.

- **Obtain a revised clearance first** whenever possible before initiating any action.
- **General contingency (no clearance):** turn **at least 30°** left or right to intercept a same-direction track **offset 9.3 km (5.0 NM)**. Squawk **7700** when able; exterior lights on; ACAS to RA mode; broadcast on **121.5 / 123.45 MHz**; advise ATC ASAP.
- **Once established on the 5 NM offset:** either descend **below FL290** and establish a **500 ft (150 m) vertical offset**, OR establish a **500 ft vertical offset** from normal levels (**1000 ft if above FL410**).
- **Weather deviation:** request via CPDLC/voice, stating **"WEATHER DEVIATION REQUIRED"** (or PAN PAN ×3). If a revised clearance cannot be obtained: broadcast on 121.5/123.45; for deviations **< 5.0 NM** remain at assigned level; for deviations **≥ 5.0 NM**, at ~5 NM from track apply a **300 ft (90 m) level change**:

  | Cleared track | Deviating LEFT | Deviating RIGHT |
  |---|---|---|
  | **EASTbound** (000°–179°) | DESCEND 300 ft | CLIMB 300 ft |
...
- **Seasonal hazards (durable):**
  - **Space weather / HF & GNSS degradation** at high latitude — polar-cap absorption (PCA) from solar proton events blacks out HF over the polar cap (NOAA scale **S3** is the historical signal to avoid/divert polar routing); high-latitude ionospheric scintillation degrades GNSS. Monitor **NOAA SWPC** (HF impacts page, D-RAP model, R/S/G scales). Carry HF + SATCOM redundancy; north of 80°N use Iridium.
  - **Jetstream / clear-air turbulence** across the OTS band; **oceanic icing** on climb/descent at the coasts.
