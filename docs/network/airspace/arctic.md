# Arctic / High-Latitude Domestic — Airspace Briefing
...
**Version** v0.1 · **Updated** 2026-07-19 · **Status** Draft — primary-verified where reached; some AIP sub-section numbers 🟧 (JS-rendered ENR pages — see §12)

> **Read-me:** Strategic transit reference for X-Plane 11, not a chart or clearance. Routings and levels are planning context — file and fly the SimBrief/current-AIRAC-validated route. This sector's defining hazards are **true-track/grid navigation**, **magnetic-compass unreliability**, **space-weather HF/GNSS degradation**, **fuel-freeze**, and **sparse diversion coverage**. Live data (NOTAMs, space-wx, SIGMET) pulled at planning (§11). Open 🟧 items in §12. Sister sector for the oceanic crossing: North Atlantic. Flag legend 🟥 hard · 🟧 caution/unverified · 🟩 normal.
...
| Domain | Flag | Note |
|---|---|---|
| Overflight / conflict-zone risk | 🟩 | Benign — no conflict zones in the polar/high-latitude domestic. Re-check live (§11). |
| Communication coverage (VHF/HF/SATVOICE) | 🟥 | Long non-VHF stretches; HF + SATCOM required, **Iridium north of 80°N**. HF subject to polar-cap absorption blackout. |
| Datalink / surveillance requirement | 🟥 | NAT DLM CPDLC+ADS-C (exceptions N of 80°N / surveilled corridors); **ADS-B mandatory IFR in BIRD FIR**; procedural position reports where non-radar. |
| Terrain / MORA / driftdown | 🟥 | **Greenland icecap dome ~10,500 ft** is the binding driftdown constraint; Gunnbjørn Fjeld 12,119 ft (E coast); Baffin 5,000–7,000 ft. Use grid-MORA on polar charts. |
| Diversion-airfield coverage | 🟥 | Very sparse and widely spaced. **BGBW closing to fixed-wing 17 APR 2026**; BGQO opening as replacement (PPR). ETOPS-equivalent diversion planning required. |
| Seasonal / environmental | 🟥 | Space-weather HF/GNSS degradation (S3+ events); extreme cold / fuel-freeze; winter darkness/icing. |
| Equipment / approval (RVSM, RNP, true-track, datalink) | 🟥 | RVSM + NAT HLA approval; RNAV10(RNP10)/RNP4; **TRUE-track reference in NDA** (all navaids/airways/procedures referenced to True, 0° declination); grid nav in high Arctic. |
...
- **Baffin Island:** Arctic Cordillera / Baffin Mountains peaks **~1,525–2,146 m (≈5,000–7,000 ft)** — relevant near CYFB. 🟩
- **Driftdown / depressurisation escape:** plan the engine-out / decompression driftdown profile to **clear the central Greenland icecap dome** — this is the limiting terrain on the route. Identify the escape route and the nearest open field (see §8) for each oceanic/icecap segment on the OFP.
...
- Iceland BI-ENR 1.8 sub-decimals and ENR 2.1 boundary coordinates (A_05-2026 page — JS-rendered).
- AIP Canada ENR section numbers for the SDA/NDA boundary text and the current NDA RNP mandate / any Northern (NRR) fixed-track structure.
- Published grid-MORA values over Greenland/Baffin (Naviair / NAV CANADA ENR 6 charts).
- Edmonton FIR radar-vs-procedural boundaries (AIP Canada ENR 1.6).
- **BGQO Qaqortoq** (new alternate, opens ~Apr 2026) — no brief yet; build if used.
- No OM C airport briefs yet for BIKF/BGSF/BGBW/BGGH/CYFB/CYYR/CYZF/CYYQ/CYEG — build if they become planned alternates.
...
| Version | Date | Change |
|---|---|---|
| v0.1 | 2026-07-19 | Ported from K Global First Officer library into current OM C airspace template (anonymised; sources/changelog split out; cross-links re-pathed). Built from Isavia BI-ENR + AIP Canada ENR (True-north briefing) + Naviair + NAT Doc 007 + NOAA SWPC. NDA true-track/grid nav, magnetic unreliability, space-wx HF/GNSS, fuel-freeze, icecap terrain, sparse diversion. Sister sector: North Atlantic. |
