<!--
================================================================================
  K Global — AIRFRAME PACK · QRH  ·  E195 Embraer 195
...
# E195 — Embraer 195 · QRH

**E195 / E-Jet family (E1 generation)** · flown as X-Crafts E195 (lead) / Flight Procedures Simulation E195 (fallback, XP12-only) 🟧 `[VERIFY — model]` · sim = X-Plane 11
**Version** v0.1 · **Updated** 2026-07-25 · **Status** Draft

> **Read-me:** K Global quick-reference for abnormal & emergency handling on X-Plane. **In a real non-normal the aircraft's EICAS/electronic checklist is authoritative** — this document supports training and quick recall and is tailored to what the approved model actually simulates (🟧). All content is re-expressed in K Global's own words from general public E-Jet type knowledge; **no verbatim OEM text**, and the source material surveyed for this type was thin on abnormal procedures, so several chapters below are built from general public turbofan/regional-jet knowledge rather than a type-specific abnormal source — flagged 🟧 throughout. Identical procedures to the common-rated E190 (see that pack's QRH) — the only type-specific difference is the stretched airframe's slightly longer field-performance figures (Dispatch §6, Ch 13). Chapters mirror ATA-style grouping. Flag legend 🟥 memory/immediate · 🟧 model-dependent · 🟩 reference. **Verify every figure `[VERIFY]` before relying on it.**
...
### Flap / slat jam
- **Recognition:** EICAS flap/slat fault. · **Actions:** stop at the safe achieved configuration, compute the higher approach speed & landing distance (Ch 13) — this stretched airframe's baseline field lengths are already slightly longer than the E190's (Dispatch §6), so the margin erodes faster — use a longer runway.
...
- **Abnormal-configuration landing distance & approach-speed increments** (flap/slat-less, reduced flight-control authority): compute via the sim/OFP performance tool for the flown configuration — do not guess; use a longer runway and a higher approach speed `[VERIFY]`. This stretched airframe's baseline TO/landing field lengths already run slightly longer than the E190's (Dispatch §6) — check runway adequacy with extra margin when a non-normal adds a further increment.
- **Single-engine diversion:** on engine loss, plan the nearest suitable diversion field promptly — this type's City-network route set (Dispatch §5) is **not typically EDTO-planned**, so there is normally no extended single-engine driftdown/critical-fuel case to work, unlike the mainline widebody EDTO fleet.
...
| Version | Date | Change |
|---|---|---|
| v0.1 | 2026-07-25 | Initial draft — chaptered E-Jet (E1/CF34-10E) abnormal/emergency reference mirrored from the common-rated E190 QRH (type-specific abnormal source thin) 🟧; stretched-airframe field-length note added to Ch 7/13; memory items 🟥 (unreliable airspeed, emergency descent, engine fire, evacuation, stall); no hard flight-envelope protection noted (Ch 0/7); no EDTO driftdown case for this network (Ch 13); figures `[VERIFY]` |
