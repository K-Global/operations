<!--
================================================================================
  K Global — AIRFRAME PACK · QRH  ·  CRJ7 Bombardier CRJ700
...
# CRJ7 — Bombardier CRJ700 · QRH

**CRJ7 / CRJ series** · flown as AD Simulations/DeltaWing CRJ-700 🟧 `[VERIFY — model]` · sim = X-Plane 11
...
- CRJ700 philosophy: **fly first**, let the EICAS present the caution/warning, then work the **electronic/paper checklist** for the ordered actions. PF flies + manages thrust/config; PM reads and actions the checklist.
...
- **Conventional flight controls 🟧:** the CRJ700 is **not** fly-by-wire — ailerons, elevator and rudder are conventionally actuated through a yoke and pedals (cable/hydraulic), and there is **no envelope-protection law**. Stall, overspeed and bank limits are **not** automatically enforced; the pilot must recognise and recover manually.
...
- **Wing/icing note 🟧:** this type has a **slatted wing** (Intro §6) — it is not the hard-wing CRJ-200. Icing/anti-ice items below are the general regional-jet contamination-discipline caveats (ground de-ice holdover, in-flight anti-ice, contamination-adjusted speeds), not a structural no-slat AOA limitation.
...
- **Notes/model 🟧:** AC ESS XFER/MAN bus-transfer logic is represented on the candidate add-on at a systems level (per its public feature notes), but depth/version behaviour should be verified before training to it `[VERIFY]`.
...
### Flap / slat jam
- **Recognition:** EICAS flap/slat fault. · **Actions:** stop at the safe achieved configuration, compute the higher approach speed & landing distance (Ch 13), use a longer runway. This type has a **slatted** wing (Intro §6) — a slat-jam case is a genuine consideration here, unlike on a hard-wing CRJ-200 (which has none to jam).
...
| Version | Date | Change |
|---|---|---|
| v0.1 | 2026-07-25 | Initial draft — chaptered CRJ700 (Pro Line 4/EICAS, conventional flight controls, T-tail) abnormal/emergency reference built largely from general public turbofan/regional-jet knowledge (type-specific abnormal source thin) 🟧; memory items 🟥 (stall/stick-shaker, unreliable airspeed, emergency descent, engine fire, evacuation); wing/icing premise corrected (slatted wing, standard contamination discipline, not hard-wing AOA case); no EDTO driftdown case for this network (Ch 13); figures `[VERIFY]` |
